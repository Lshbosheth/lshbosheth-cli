...
class Creator {
  ...
  async download(repo, tag) {
    // 模板下载地址
    const templateUrl = `Lshbosheth/${repo}${tag ? "#" + tag : ""}`;
  ...
  async setUpPackage() {
    const filePath = this.target + '\\package.json';
    try {
      const source = fs.readFileSync(filePath, {
        encoding: 'utf-8'
      })
      let sourceStr = JSON.stringify(source)
      sourceStr = sourceStr.replace('project-template', this.name)
      fs.writeFileSync(filePath, JSON.parse(sourceStr), 'utf-8');
    } catch (e) {
      console.log(e)
    }
  }
